...
export default function MyMessages() {
  ...
  useEffect(() => {
    ...
    getStudentByEmail(user.email).then((rec) => {
      if (!rec) setMissingProfile(true);
      else setStudent(rec);
    }).catch(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToMessages(student.id, student.instructorId, (msgs) => {
      setMessages(msgs);
      const unread = msgs.filter((m) => m.sender !== student.id && !m.read);
      Promise.all(unread.map((m) => markMessageRead(m.id).catch(() => {})));
    });
    return () => unsubscribe();
  }, [student]);

  useEffect(() => {
    if (!student?.id || !student?.instructorId) return;
    const typingRef = doc(db, "typing", `${student.instructorId}_${student.id}`);
    const unsub = onSnapshot(typingRef, (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        setInstructorTyping(data.isTyping && Date.now() - (data.timestamp || 0) < 10000);
      } else {
  ...
  const setTyping = useCallback((isTyping) => {
    if (!student?.id || !student?.instructorId) return;
    const typingRef = doc(db, "typing", `${student.id}_${student.instructorId}`);
    if (isTyping) {
  ...
  async function handleSend(e) {
    e.preventDefault();
    if (!text.trim() || !student?.id) return;
    ...
    setSending(true);
    try {
      await sendMessage({ sender: student.id, receiver: student.instructorId, text: messageText, instructorId: student.instructorId });
    } catch {
  ...
  async function handleClearChat() {
    const myMsgIds = messages.filter(m => m.sender === student.id).map(m => m.id);
    ...
    return (
      <div className="conversation-page">
        <div className="chat-empty">
          ...
          <p>Your instructor needs to add your email address to your student record first.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="conversation-page">
      <div className="conv-header">
        ...
        <div className="conv-header-info">
          <div className="conv-header-avatar-initial">I</div>
          <div>
            <h2>Your Instructor</h2>
            <span className="conv-header-status">
              {instructorTyping ? "typing..." : "Messages with your instructor"}
            </span>
          </div>
        </div>
        {messages.length > 0 && (
      ...
      <div className="chat-messages" onClick={() => setSelectedId(null)}>
        {messages.length === 0 ? (
          <div className="chat-empty">
            ...
            <p>Say hello when you're ready!</p>
          </div>
        ) : (
          grouped.map((item, i) =>
            item.type === "date" ? (
              ...
              <div
                key={item.data.id}
                className={"chat-bubble " + (item.data.sender === student.id ? "sent" : "received") + (selectedId === item.data.id ? " selected" : "")}
