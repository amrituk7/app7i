...
function TestBookingSection({ student, onUpdate }) {
  const { showToast } = useToast();
  const [editing, setEditing] = useState(false);
  const [test, setTest] = useState({
    testDate: student.testDate || "",
    testTime: student.testTime || "",
    testCentre: student.testCentre || "",
    testRef: student.testRef || "",
    candidateNumber: student.candidateNumber || "",
    testFeeStatus: student.testFeeStatus || "unpaid"
  });

  async function handleSave() {
    try {
      await updateStudent(student.id, test);
      onUpdate(test);
      setEditing(false);
      showToast("Test booking saved", "success");
    } catch {
      showToast("Failed to save", "error");
    }
  }

  const inp = { width: "100%", padding: "8px 12px", borderRadius: "6px", border: "1px solid #ddd", fontSize: "14px", boxSizing: "border-box" };

  if (!editing && !student.testDate) {
    return (
      <div>
        <p style={{ color: "#6b7280", fontSize: "14px", marginBottom: "10px" }}>No test booked yet.</p>
        <button onClick={() => setEditing(true)} style={{ background: "#2563eb", color: "white", border: "none", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "14px" }}>
          Add Test Booking
        </button>
      </div>
    );
      ...
      <div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px" }}>
          {[
            ...
            ["Booking Ref", student.testRef],
            ["Candidate No.", student.candidateNumber],
            ["Fee Status", student.testFeeStatus || "unpaid"]
          ].map(([label, val]) => val ? (
            <div key={label} style={{ padding: "6px 0" }}>
              <div style={{ fontSize: "12px", color: "#6b7280", fontWeight: "600" }}>{label}</div>
              <div style={{ fontSize: "14px", textTransform: "capitalize" }}>{val}</div>
            </div>
          ) : null)}
        </div>
        <button onClick={() => setEditing(true)} style={{ marginTop: "10px", background: "#2563eb", color: "white", border: "none", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "13px" }}>
          Edit Test Details
        </button>
      </div>
    );
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 12px" }}>
        <div><label style={{ fontSize: "13px", fontWeight: "600" }}>Test Date</label><input style={inp} type="date" value={test.testDate} onChange={e => setTest({ ...test, testDate: e.target.value })} /></div>
        <div><label style={{ fontSize: "13px", fontWeight: "600" }}>Test Time</label><input style={inp} type="time" value={test.testTime} onChange={e => setTest({ ...test, testTime: e.target.value })} /></div>
        <div><label style={{ fontSize: "13px", fontWeight: "600" }}>Test Centre</label><input style={inp} placeholder="e.g. Goodmayes" value={test.testCentre} onChange={e => setTest({ ...test, testCentre: e.target.value })} /></div>
        <div><label style={{ fontSize: "13px", fontWeight: "600" }}>Booking Reference</label><input style={inp} placeholder="DVSA ref" value={test.testRef} onChange={e => setTest({ ...test, testRef: e.target.value })} /></div>
        <div><label style={{ fontSize: "13px", fontWeight: "600" }}>Candidate Number</label><input style={inp} placeholder="Candidate #" value={test.candidateNumber} onChange={e => setTest({ ...test, candidateNumber: e.target.value })} /></div>
        <div>
          <label style={{ fontSize: "13px", fontWeight: "600" }}>Fee Status</label>
          <select style={inp} value={test.testFeeStatus} onChange={e => setTest({ ...test, testFeeStatus: e.target.value })}>
      ...
      <div style={{ display: "flex", gap: "8px", marginTop: "12px" }}>
        <button onClick={handleSave} style={{ background: "#10b981", color: "white", border: "none", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "14px", fontWeight: "600" }}>Save</button>
        <button onClick={() => setEditing(false)} style={{ background: "#f3f4f6", border: "1px solid #ddd", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
      </div>
    </div>
  );
}
...
export default function StudentProfile() {
  ...
  function whatsappLink(phone, name, instructor) {
    ...
    const msg = encodeURIComponent(`Hi ${name || ""}, just a reminder about your upcoming driving lesson with ${instructor}. See you soon!`);
  ...
  return (
    <div className="student-profile-page">
          ...
          <div className="student-profile-heading">
            ...
            <div className="student-meta-chips">
              ...
              {student.language && student.language !== "en" && (
                <span className="student-meta-chip">{getLanguage(student.language).label}</span>
              )}
            ...
            <div className="student-action-row" style={{ marginTop: "18px" }}>
              ...
              {student.phone && (
                <a
                  href={whatsappLink(student.phone, student.name, instructorName)}
                  target="_blank"
      ...
      <section className="student-card-panel" style={{ marginBottom: "20px" }}>
        <h2 style={{ marginBottom: "12px" }}>Test Booking</h2>
        {student.testPassed ? (
          <div style={{ color: "#065f46", fontWeight: "600" }}>Test passed on {student.testPassedDate || "unknown date"}</div>
        ) : (
          <TestBookingSection student={student} onUpdate={(data) => setStudent(prev => ({ ...prev, ...data }))} />
        )}
      </section>
                ...
                {recentMessages.map((message) => (
                  <div
                    ...
                    className={`student-message-item ${
                      message.sender === user?.uid ? "instructor" : ""
                    }`}
                  >
                    <strong>{message.sender === user?.uid ? "You" : student.name}</strong>
